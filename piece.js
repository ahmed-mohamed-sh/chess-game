...
const getMovesTop = (piece) => {
  const movesTop = [];
  for (let move = piece.position + 10; move <= 88; move += 10)
    movesTop.push(move);
  return movesTop;
};
...
const getMovesBottom = (piece) => {
  const movesBottom = [];
  for (let move = piece.position - 10; move >= 11; move -= 10)
...
const getBishopAllowedMoves = (bishop) => {
  return [
    getMovesTopRight(bishop),
    getMovesTopLeft(bishop),
    getMovesBottomRight(bishop),
    getMovesBottomLeft(bishop),
  ];
};
