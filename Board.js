const startBoard = (
  game,
  options = { playAgainst: "human", aiColor: "black", aiLevel: "dumb" }
) => {
  ...
  const clearSquares = () => {
    board
      .querySelectorAll(".allowed")
      .forEach((allowedSquare) => allowedSquare.classList.remove("allowed"));
  ...
  const setLastMoveSquares = (from, to) => {
    document
      ...
      .forEach((lastMoveSquare) =>
        lastMoveSquare.classList.remove("last-move")
      );
  ...
  const startTurn = (turn) => {
    ...
    if (
      gameState !== "checkmate" &&
      options.playAgainst === "ai" &&
      turn === options.aiColor
    ) {
...
const startNewGame = () => {
  document
    ...
    .forEach((scene) => scene.classList.remove("show"));

  const playAgainst = document.querySelector(
    'input[name="oponent"]:checked'
  ).value;
  const humanColor = document.querySelector(
    'input[name="human_color"]:checked'
  )?.value;
